-- Generated by build-item-game; sourceHash bb83226f5f7dc9f0fb108c53e97f3fbd2de4ea90e6d12e80e79940ca3b2fd6cf
local G=args._G
...
if old and old.bundleId~="bb83226f5f7dc9f0fb108c53e97f3fbd2de4ea90e6d12e80e79940ca3b2fd6cf" and old.current then old.current.stop("engine_update") end
local E=old
if not E or E.bundleId~="bb83226f5f7dc9f0fb108c53e97f3fbd2de4ea90e6d12e80e79940ca3b2fd6cf" then
 E={bundleId="bb83226f5f7dc9f0fb108c53e97f3fbd2de4ea90e6d12e80e79940ca3b2fd6cf",packages={["particles3d"]=true,["runtime"]=true,["scene3d"]=true,["surface"]=true}}
-- module: core
...
return function(E, G)
    ...
    E.VERSION, E.API_VERSION = "0.4.0", 1
    E.util = {}
...
return function(E,G)
    ...
    function E.newScene3D(options)
        options=options or {}
        local self={nodes={},order={},revision=0,faceCount=0,closed=false}
        ...
        function self.renderList()
            ...
            for _,n in ipairs(self.order)do if n.worldVisible and n.mesh then
                if n.drawWorld~=n.worldVersion or n.drawCamera~=cameraVersion or n.drawTint~=n.tintVersion then
                    ...
                    for i,v in ipairs(n.vertices)do
                    ...
                    for fi,face in ipairs(n.mesh.faces)do
                        local a=n.vertices[face[1]];local normal=n.normals[fi]
                        if face.doubleSided or normal.x*(cp.x-a.x)+normal.y*(cp.y-a.y)+normal.z*(cp.z-a.z)>1e-9 then
                            ...
                            if #points>=3 then
                                local color=face.color or {1,1,1,1};local shade=face.unlit and 1 or (.48+.52*math.max(0,M.dot(normal,light)))
...
function U.new(ctx,art)
    ...
    function self.draw(s,actions)
        ...
        text("time-label","用时",475,15,65,20,12,colors.muted)
        text("time",time(s.elapsed),475,34,90,27,22)
        text("moves-label","转动",583,15,65,20,12,colors.muted)
        text("moves",tostring(s.moves),583,34,90,27,22)
        ...
        for n=2,7 do button("order"..n,n.." 阶",84+(n-2)*75,80,66,32,function()actions.order(n)end,not s.busy and not s.benchmark,n==s.n)end
        ...
        text("status",s.fireworksOpen and s.fireworks.message or s.message,30,540,900,26,14,s.won and {.45,.9,.65,1} or colors.white)
...
return function(ctx,modules)
    ...
    local scene,ui,active,gesture,confirmation,fireworks
    ...
    local function startTurn(move,kind)
        if active or ctx.session.isPaused() or confirmation or fireworks.testing then return false end
    ...
    local function requestFresh(n,scramble)
        if active or fireworks.testing then return end
    ...
    actions.fireworks=function()s.fireworksOpen=not s.fireworksOpen;display()end
    ...
    local function pointer(event)
        ...
        if event.phase=="down" then
            ...
            if event.button=="LeftButton" and hit and not active and not s.replaying then selectedPoint(hit)end
        elseif event.phase=="move" and gesture then
            if event.button=="RightButton" or not gesture.hit then
        ...
        elseif event.phase=="up" and gesture then
            ...
            if event.button~="LeftButton" or not g.hit or active or s.replaying then return end
    ...
    return {
        ...
        onFixedUpdate=function(dt,input)
            fireworks.update(dt)
            if s.started and not s.won and not s.replaying and not fireworks.testing then s.elapsed=s.elapsed+dt end
            if active then
            ...
            if not confirmation and not active and not s.replaying and not fireworks.testing and not s.fireworksOpen then
        ...
        onSave=function()
            return {schema=1,n=s.n,history=s.history,moves=s.moves,elapsed=s.elapsed,started=s.started,won=s.won,
                challenge=s.challenge,assisted=s.assisted,bests=s.bests,yaw=s.yaw,pitch=s.pitch,distance=s.distance,axis=s.axis,layer=s.layer}
        end,
