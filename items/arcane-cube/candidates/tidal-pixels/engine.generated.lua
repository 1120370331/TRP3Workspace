-- Generated by build-item-game; sourceHash 0374bbd7102cddb50a804e56824fc59ebdbd55ebcaf87b4719feb336f4fc67e2
local G=args._G
...
if old and old.bundleId~="0374bbd7102cddb50a804e56824fc59ebdbd55ebcaf87b4719feb336f4fc67e2" and old.current then old.current.stop("engine_update") end
local E=old
if not E or E.bundleId~="0374bbd7102cddb50a804e56824fc59ebdbd55ebcaf87b4719feb336f4fc67e2" then
 E={bundleId="0374bbd7102cddb50a804e56824fc59ebdbd55ebcaf87b4719feb336f4fc67e2",packages={["particles3d"]=true,["pixel-materials"]=true,["runtime"]=true,["scene3d"]=true,["surface"]=true}}
-- module: core
...
return function(E, G)
    ...
    E.VERSION, E.API_VERSION = "0.4.1", 1
    E.util = {}
    ...
    function P.bake(mesh,bindings,textures,budget)
        ...
        for fi,face in ipairs(mesh.faces)do
            ...
            if not binding then
                ...
                if quadFast then
                    for _,p in ipairs(uv)do if (p.u~=0 and p.u~=1) or (p.v~=0 and p.v~=1)then quadFast=false end end
                    ...
                    if uv[1].u+uv[3].u~=uv[2].u+uv[4].u or uv[1].v+uv[3].v~=uv[2].v+uv[4].v then quadFast=false end
                end
                for ti=2,quadFast and 2 or #face-1 do
                    ...
                    for _,rect in ipairs(texture.rectangles)do
                        ...
                        local poly={rotate(l,t,turns),rotate(r,t,turns),rotate(r,bottom,turns),rotate(l,bottom,turns)}
                    ...
                    for fi,face in ipairs(displayMesh.faces)do
                        ...
                        if source.doubleSided or normal.x*(cp.x-a.x)+normal.y*(cp.y-a.y)+normal.z*(cp.z-a.z)>1e-9 then
                            ...
                            if #points>=3 then
                                ...
                                faces[#faces+1]={points=points,depth=depth/#points,order=n.ordinal*4096+fi,id=n.id,face=fi,key="g:"..n.id..":"..fi,
...
return function(E,G)
    function E.newWoWScene3D(session,parent,options,cache)
        ...
        for _,h in ipairs(cache.pool)do free[#free+1]=h end
        ...
        local function acquire(key)
            ...
            if not h then
                ...
                assert(texture.SetVertexOffset,"3D rendering requires Texture:SetVertexOffset")
                if texture.SetSnapToPixelGrid then texture:SetSnapToPixelGrid(false);texture:SetTexelSnappingBias(0)end
                texture:SetTexture("Interface\\Buttons\\WHITE8X8")
                h={frame=frame,texture=texture};cache.pool[#cache.pool+1]=h
            end
            byKey[key]=h;return h
        end
        local function polygon(points,color,material,key,face)
            if drawn>=4096 then dropped=dropped+1;return end
            drawn=drawn+1;local h=acquire(key)
            local level=root:GetFrameLevel()+drawn
            if h.level~=level then h.frame:SetFrameLevel(level);h.level=level end
            if not material and h.geometryFace==face then h.frame:Show();return end
            ...
            if h.x~=minX or h.y~=minY then
                h.frame:ClearAllPoints();h.frame:SetPoint("TOPLEFT",root,"TOPLEFT",minX,-minY);h.x,h.y=minX,minY
            end
            if h.w~=w or h.h~=ht then
                h.frame:SetSize(w,ht);h.texture:SetSize(w,ht);h.w,h.h=w,ht
            end
            local t=h.texture
            if not h.anchored then t:ClearAllPoints();t:SetPoint("TOPLEFT",h.frame,"TOPLEFT",0,0);h.anchored=true end
        ...
        function scene.render(alpha)
            ...
            input:ClearAllPoints();input:SetPoint("TOPLEFT",root,"TOPLEFT",v.x,-v.y);input:SetSize(v.w,v.h);input:Show()
            ...
            for key,h in pairs(byKey)do
                if not desired[key]then h.frame:Hide();byKey[key]=nil;free[#free+1]=h end
            end
            ...
            for i=1,count do
                ...
                polygon(c.points,c.face.color,c.particle and c.face or nil,c.key,c.face)
        ...
        function scene.stats()
            local out=coreStats();out.scene3DDraws=drawn;out.scene3DPooledQuads=#cache.pool;out.scene3DDropped=dropped
        ...
        function scene.dispose()
            ...
            for _,h in ipairs(cache.pool)do h.frame:Hide()end
            drawn=0;coreDispose()
        end
        return scene
    end
...
function F.new(ctx,scene,getOrder)
    ...
    local function phase()
        ...
        ctx.perf.begin("fireworks."..({"baseline","low","normal","high"})[stage],
            {kind="particles3d",order=getOrder(),pattern="five_types",intensity=stage-1,
...
function A.mesh(c,api,puzzle)
    local M=api.math;local mesh=api.boxMesh(.93,{.065,.073,.082,1})
...
function U.new(ctx,art)
    ...
    function self.draw(s,actions)
        ...
        text("mode",s.challenge and (s.assisted and "辅助练习" or "计时挑战") or "自由练习",552,85,120,22,13,colors.gold,"RIGHT")
        ...
        if s.materialsOpen then
            ...
            text("mat-count","色块合并 "..m.rectangles.."    绘制面 "..m.stats.scene3DDraws,30,510,625,22,12,colors.muted)
...
return function(ctx,modules)
    ...
    local s={schema=1,n=3,history={},moves=0,elapsed=0,started=false,won=false,challenge=false,assisted=false,bests={},
        yaw=.63,pitch=.42,distance=7.8,axis="y",layer=3,message="转动视角，或打乱魔方开始挑战。"}
    ...
    local function rebuild()
        ...
        for _,c in ipairs(puzzle.cubies)do
            local spec=pose(c);spec.mesh=art.mesh(c,ctx.scene3d,P);spec.tag=c.id
    ...
    return {
        onStart=function()
            ...
            fireworks=modules.fireworks.new(ctx,scene,function()return s.n end)
        ...
        onSave=function()
            ...
            return {schema=1,n=s.n,history=s.history,moves=s.moves,elapsed=s.elapsed,started=s.started,won=s.won,
                challenge=s.challenge,assisted=s.assisted,bests=s.bests,yaw=camera.yaw,pitch=camera.pitch,distance=camera.distance,axis=s.axis,layer=s.layer}
        end,
